...
const ProductCard = ({
  ...
  const handleCheckLocale = (firstCheck , secondCheck) => activeLocale === "en" ? firstCheck : secondCheck
  ...
  return (
    ...
    >
      <div className={`${HeightProductCard} relative z-10 overflow-hidden rounded-2xl rounded-b-none bg-gray-200`}>
        <Image
          width={2000}
          height={20}
          src={src}
        ...
        {haveDicount ? <div key="" className={`flex justify-start items-end ml-1 pb-1`}>
                  <DicountBadge
                    width={80}
                    dicount={dicountPrec}
                    text={textDicount}
      ...
      <div className={`z-0 pt-2 pb-4 text-xs p-3 w-full mb-1`}>
        <p className={`text-xs font-bold text-gray-900 ${!warning ? "mb-5" : ""} ${classTitleProductCard}`}>
            {title}
        </p>

          <div className="mt-1 flex  items-start">
            <p className={`mt-1 text-xs text-primary-600 text-base flex gap-1 ${classWarningProductCard} ${handleCheckLocale("pr-1" , "pl-1")} `}>
              
              <span className={`inline-block ${handleCheckLocale("order-2" , "order-1")}`}>
                {warning ?
                  <Image
                    width={15}
                    height={15}
              ...
              <span className={`inline-block ${handleCheckLocale("order-1" , "order-2")}`}>{warning}</span>
            </p>
          </div>
        {/* {warning ?  */}
	       {/*  <div className="mt-1 flex  items-start"> */}
