	      ...
	      >
				{data?.data?.map((product, key) => (
					<SwiperSlide className="flex flex-col justify-center" key={key}>
						<ProductCard
							src={product?.img}
							alt={activeLocale?.includes("en") ? product?.english_name : product?.arabic_name}
							classTitleProductCard="titleProductCard"
							classWarningProductCard="warningProductCard"
							titleBadge={activeLocale?.includes("en") ? product?.english_label : product?.arabic_label}
							title={activeLocale?.includes("en") ? product?.english_name : product?.arabic_name}
							warning={`${t("Offer_ends_until")} ${product?.expire?.replaceAll("-" , "/")}`}
							haveFav={false}
